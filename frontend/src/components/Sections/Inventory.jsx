...
import {
    ...
    ChevronUp, Download, DollarSign, Store, Layers, Trash2
} from 'lucide-react';
...
const Inventory = () => {
    ...
    const handleImageUpload = async (e, productId) => {
        ...
        try {
            const res = await fetch(`http://localhost:3001/api/inventory/${productId}/upload`, {
                method: 'POST',
                body: formData,
            });
            if (res.ok) {
    ...
    useEffect(() => {
        console.log("Inventory: activeStore changed:", activeStore);
        if (activeStore) {
            fetchData();
        } else {
            console.log("Inventory: No activeStore, clearing products");
            setProducts([]);
        }
    }, [activeStore]);
    ...
    const fetchData = async () => {
        try {
            console.log("Inventory: fetching data for store:", activeStore?.id);
            setLoading(true);
            const storeIdParam = activeStore ? `storeId=${activeStore.id}` : '';

            const [resProducts, resStores] = await Promise.all([
                fetch(`http://localhost:3001/api/inventory?${storeIdParam}`),
        ...
        try {
            if (deleteConfirm.isGlobal) {
                ...
                await Promise.all(productsToDelete.map(p =>
                    fetch(`http://localhost:3001/api/inventory/${p.id}`, { method: 'DELETE' })
                ));
            } else {
                await fetch(`http://localhost:3001/api/inventory/${deleteConfirm.product.id}`, { method: 'DELETE' });
            }
    ...
    return (
        ...
        <div className="p-6 md:p-10 space-y-8 animate-in fade-in duration-500 bg-[#F9FAFB] dark:bg-dark-bg h-full overflow-y-auto custom-scrollbar">
            ...
            <div className="no-print grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                <button
                    onClick={() => setTransferModal({ isGeneral: true })}
                            ...
                            <table className="w-full text-left border-separate border-spacing-0">
                                ...
                                <tbody className="divide-y divide-gray-50 dark:divide-white/5">
                                    {loading ? (
                                        ...
                                        return (
                                            <tr key={p.id || idx} className="hover:bg-blue-50/30 dark:hover:bg-blue-500/5 transition-colors group">
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center space-x-4">
                                                        ...
                                                        <span className="font-bold text-gray-900 dark:text-white tracking-tight">{p.name}</span>
                                                    </div>
                                                </td>

                                                <td className="px-4 py-4 text-xs font-bold text-gray-500 max-w-[120px] truncate">
                                                    {p.category}
                                                </td>
                                                ...
                                                <td className="px-2 py-4 text-center">
                                                    <div className={`w-3 h-3 rounded-full mx-auto shadow-sm ${status.bg} border-2 border-white dark:border-dark-card`} />
                                                </td>

                                                <td className="px-4 py-4 text-center flex items-center justify-center gap-1">
                                                    <button
                                                        onClick={() => setAdjustModal({ product: p })}
                                                        className="w-8 h-8 rounded-full bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 flex items-center justify-center hover:bg-blue-600 hover:text-white transition-all transform hover:scale-110 active:scale-95 shadow-sm"
                                                    >
                                                        <Plus className="w-4 h-4" />
                                                    </button>
                                                    <button
                                                        onClick={() => setDeleteConfirm({ product: p, isGlobal: false })}
                                                        className="w-8 h-8 rounded-full bg-gray-50 dark:bg-white/5 text-gray-400 flex items-center justify-center hover:bg-red-50 dark:hover:bg-red-500/10 hover:text-red-500 transition-all"
                                                    >
                    ...
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 backdrop-blur-sm bg-black/50 overflow-hidden">
                        <div className="bg-white dark:bg-dark-card w-full max-w-md rounded-[32px] overflow-hidden shadow-2xl border border-gray-100 dark:border-white/10 animate-in fade-in zoom-in duration-200">
                            ...
                            <form onSubmit={async (e) => {
                                ...
                                try {
                                    const res = await fetch(`http://localhost:3001/api/inventory/${adjustModal.product.id}/adjust`, {
                                        method: 'POST',
                    ...
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 backdrop-blur-sm bg-black/50 overflow-hidden">
                        <div className="bg-white dark:bg-dark-card w-full max-w-md rounded-[32px] overflow-hidden shadow-2xl border border-gray-100 dark:border-white/10 animate-in fade-in zoom-in duration-200">
                            ...
                            <form onSubmit={async (e) => {
                                ...
                                try {
                                    const res = await fetch('http://localhost:3001/api/inventory/transfer', {
                                        method: 'POST',
                                        headers: { 'Content-Type': 'application/json' },
                                        body: JSON.stringify(data)
                                    });
                                ...
                                <div className="space-y-4">
                                    ...
                                    {transferModal.isGeneral ? (
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <select required name="productId" className="w-full bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white border border-gray-200 dark:border-white/10 rounded-2xl p-4 outline-none focus:ring-2 focus:ring-indigo-500 transition-all font-bold">
                                                <option value="" className="bg-white dark:bg-gray-900 text-gray-900 dark:text-white">Seleccionar Producto...</option>
                                                {products
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <select required name="targetStoreId" className="w-full bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white border border-gray-200 dark:border-white/10 rounded-2xl p-4 outline-none focus:ring-2 focus:ring-indigo-500 transition-all font-bold">
                                            ...
                                            {stores
                                                ...
                                                .filter(s => {
                                                    const srcStoreId = transferModal.product?.storeId || activeStore?.id;
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <input required step="0.01" type="number" name="quantity" max={transferModal.product?.stock} className="w-full bg-gray-50 dark:bg-black/20 border border-gray-200 dark:border-white/10 rounded-2xl p-4 outline-none focus:ring-2 focus:ring-indigo-500 transition-all font-mono font-bold" placeholder="0.00" />
                                        <p className="text-[10px] font-bold text-gray-400 text-right">Disponible: {transferModal.product?.stock ?? '-'}</p>
                                    </div>
