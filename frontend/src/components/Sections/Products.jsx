...
const Products = () => {
    ...
    const fetchStats = async () => {
        try {
            setLoading(true);
            const res = await fetch(`http://localhost:3001/api/products/dashboard-stats?month=${selectedMonth}&year=${selectedYear}&storeId=${activeStore?.id || ''}`);
    ...
    const fetchAnalytics = async () => {
        try {
            const res = await fetch(`http://localhost:3001/api/products/analytics?month=${selectedMonth}&year=${selectedYear}&storeId=${activeStore?.id || ''}`);
            const data = await res.json();
            setAnalytics(data);
        } catch (error) {
    ...
    const fetchInventory = async () => {
        try {
            const res = await fetch(`http://localhost:3001/api/inventory?storeId=${activeStore?.id || ''}`);
            const data = await res.json();
            setInventory(data);
        } catch (error) {
    ...
    const handleCreateBundle = async () => {
        try {
            const res = await fetch('http://localhost:3001/api/products/bundle', {
                method: 'POST',
    ...
    return (
        <div className="h-screen overflow-y-auto bg-light-bg dark:bg-dark-bg p-6 space-y-6 no-scrollbar transition-colors duration-300">
            ...
            {isBundleModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in">
                    <div className="bg-white dark:bg-dark-surface w-full max-w-4xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
